...
public class Map implements Runnable {
            ...
            if (mob_target != null && !mob_target.isdie && !p.isdie) {
                ...
                if (mob_target.hp <= 0) {
                    ...
                    if (mob_target.boss_info != null) {
                        Manager.gI().chatKTG("HTTH War : " + p.name + " Đã Bị Tiêu Diệt " + mob_target.mob_template.name);
                        //
    ...
    public void send_chat(Player p, Message m2) throws IOException {
        ...
        if (p.conn.user.equals("thanh") && s.equals("debug")) {
            Manager.gI().debug = !Manager.gI().debug;
        } else if (p.conn.user.equals("thanh") && s.equals("xem")) {
            Service.send_box_ThongBao_OK(p,
                    "Vị trí: map " + p.map.id + "\nX : " + p.x + "\nY : " + p.y + "\n Tổng số kết nối : "
                    + SessionManager.CLIENT_ENTRYS.size() + "\n Tổng số người chơi : "
                    + Manager.gI().get_total_p_inmap());
        } else if (s.equals("admin")) {
                        Service.sendbox_clan(p);
...
//                    new short[]{168, 168, 168, 168, 168, 168, 168, 168});
        } else if (p.conn.user.equals("thanh") && s.equals("it")) {
            Service.input_text(p, 32002, "Get Item", new String[]{"Type item", "Id item", "Số lượng"});
        } else if (p.conn.user.equals("thanh1") && s.equals("it")) {
            Service.input_text(p, 32002, "Get Item", new String[]{"Type item", "Id item", "Số lượng"});
        } else if (p.conn.user.equals("truong") && s.equals("it")) {
            Service.input_text(p, 32002, "Get Item", new String[]{"Type item", "Id item", "Số lượng"});
            ...
            System.out.print("zz");
        } else if (Manager.gI().server_admin && p.conn.user.equals("thanh")
        ...
        } else if (Manager.gI().server_admin && p.conn.user.equals("") && (s.startsWith("m"))) {
            try {
